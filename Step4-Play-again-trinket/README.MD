...
The image file for the ```yes``` turtle is ```'yes.gif'```. After the code which sets the shape of the 'choose' turtle add a line to set the shape for the ```yes``` turtle, then another line to set the shape for the ```no``` turtle. (Follow the same pattern used to set the shapes for the 'choose' turtles.)
...
Go to the **MAIN CODE** section and put a ```#``` sign at the beginning of the line ```make_choice()```. This will turn this line into a comment, so it won't run.
...
Next we need to set up ```onclick()``` functions for the two new turtles.
...
In the section of the code which starts with the comment lines for **FUNCTIONS** we will now add two new functions. Put these after the three click functions for the 'choose' turtles. The first is
...
Here we have made a new global variable called ```new_game```, which is a *Boolean* variable, meaning it can have only two values: ```True``` aornd ```False```. If the player clicks the ```yes``` turtle we will set this variable to ```True```. The last line in this function is just a test line, like we made before - if you click the turtle it will print 'True' in the text window, just to show it's working properly.
