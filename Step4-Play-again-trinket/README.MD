# Step 4 - Playing again - trinket or standalone Python
...
### Make turtles for the choosing *yes* or *no* for playing again
...
We want to send the yes and no turtles to different places on the screen, so you will have to choose x and y coordinates to use as parameters in a ```goto()``` function for each of the yes and no turtles.
...
1. In the section **SETUP** find the two lines you added to *show* the ```yes``` and ```no``` turtles and  ***remove*** these lines.
...
- next, the function will display the ```yes``` and ```no``` turtles with some text asking whether you want to play again, and also set these turtles to be ready for clicking

- then the function will check which of the ```yes``` or ```no``` turtles has been clicked:

  - if the player clicked the ```no``` turtle the game will close

  - if the player clicked the ```yes``` turtle we want to clear the screen and restart the game.
...
### Calling the new function from inside another function
...
Also, once the game is finished you might want to pause for a couple of seconds before asking whether to play again. So now, find the function ```play_game()```, and add two lines at the end of the function (don't forget to indent them, so they will be part of the function):
...
Finally we want to set these two turtles up to be clickable. For this we remove two lines we previously put in the **MAIN CODE** section as temporary lines, and add them (indented) to the end of the ```play_again()``` function. Here is the line for the ```yes``` turtle:
...
If you have followed all the instructions in step 4 you can check what code should look like [here](ex_step4.md).

### Next step

[Go to Step 5 for trinket or standalone Python](../Step5-Keeping-score-trinket/README.md)
